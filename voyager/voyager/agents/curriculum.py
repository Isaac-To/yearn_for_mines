...
class CurriculumAgent:
    def __init__(
        ...
        if resume:
            ...
            import shutil, os
            vdb_path = f"{ckpt_dir}/curriculum/vectordb"
    ...
    def propose_next_ai_task(self, *, messages, max_retries=5):
        ...
        t0 = time.time()
        llm_msg = self.llm(messages)
        U.log_llm_call(
            agent="curriculum",
            phase="propose_next_ai_task",
            ai_message=llm_msg,
            elapsed_sec=time.time() - t0,
            ckpt_dir=self.ckpt_dir,
        )
        curriculum = llm_msg.content
    ...
    def run_qa_step1_ask_questions(self, *, events, chest_observation):
        ...
        t0 = time.time()
        qa_msg = self.qa_llm(messages)
        U.log_llm_call(
            agent="curriculum_qa",
            phase="step1_ask_questions",
            ai_message=qa_msg,
            elapsed_sec=time.time() - t0,
            ckpt_dir=self.ckpt_dir,
        )
        qa_response = qa_msg.content
        try:
    ...
    def run_qa_step2_answer_questions(self, question):
        ...
        U.log_llm_call(
            agent="curriculum_qa",
            phase="step2_answer_question",
            ai_message=qa_msg,
            elapsed_sec=time.time() - t0,
            ckpt_dir=self.ckpt_dir,
        )
        qa_answer = qa_msg.content
